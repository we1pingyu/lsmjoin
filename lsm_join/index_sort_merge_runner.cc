...
int main(int argc, char* argv[]) {
  ...
  clock_gettime(CLOCK_MONOTONIC, &t1);
  if (index_type == "lazy")
    build_lazy_index(db_r, index_r, R.data(), vector<uint64_t>(), r_tuples,
                     VALUE_SIZE, SECONDARY_SIZE, PRIMARY_SIZE);
  else if (index_type == "eager")
    build_eager_index(db_r, index_r, R.data(), vector<uint64_t>(), r_tuples,
                      VALUE_SIZE, SECONDARY_SIZE, PRIMARY_SIZE);
  else
    build_composite_index(db_r, index_r, R.data(), vector<uint64_t>(), r_tuples,
                          VALUE_SIZE, SECONDARY_SIZE, PRIMARY_SIZE);
  clock_gettime(CLOCK_MONOTONIC, &t2);
  ...
  if (index_type == "lazy" || index_type == "eager") {
    while (it_r->Valid() && it_s->Valid()) {
      ...
      if (temp_r_key == temp_s_key) {
        ...
        for (auto x : value_split) {
          s = db_r->Get(ReadOptions(), x.substr(0, PRIMARY_SIZE), &tmp);
    ...
    while (it_r->Valid() && it_s->Valid()) {
      ...
      if (temp_r_key == temp_s_key) {
        clock_gettime(CLOCK_MONOTONIC, &t3);
        s = db_r->Get(ReadOptions(), temp_r_value, &value_r);
        if (s.ok() && value_r.substr(0, SECONDARY_SIZE) == temp_r_key) count1++;
        ...
        while (it_r->Valid()) {
          ...
          if (temp_r_key == tmp) {
            s = db_r->Get(ReadOptions(), temp_r_value, &value_r);
            if (s.ok() && value_r.substr(0, SECONDARY_SIZE) == temp_r_key)
              count1++;
          } else
            break;
        }
