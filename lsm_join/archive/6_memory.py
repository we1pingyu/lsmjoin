...
for i, M in enumerate(unique_M_MB):
    for theory in [0, 1]:
        for j, label in enumerate(unique_labels):
            ...
            sum_join_time = buffer_size[
                (buffer_size["M_MB"] == M)
                & (buffer_size["label"] == label)
                & (buffer_size["theory"] == theory)
            ]["sum_join_time"].values
            cache_hit_rate = buffer_size[
                (buffer_size["M_MB"] == M)
                & (buffer_size["label"] == label)
                & (buffer_size["theory"] == theory)
            ]["cache_hit_rate"].values
            index_build_time = buffer_size[
                (buffer_size["M_MB"] == M)
                & (buffer_size["label"] == label)
                & (buffer_size["theory"] == theory)
            ]["sum_index_build_time"].values
            ...
            # 计算条形图的位置
            x_pos = (
                i * (len(unique_labels) * bar_width * 2 + group_gap)
                + j * bar_width * 2
                + theory * bar_width
            )
            # 绘制条形图
...
for ax in [ax1, ax2, ax3]:
    ax.set_xticks(
        [
            i * (len(unique_labels) * bar_width * 2 + group_gap)
            + (len(unique_labels) - 1) * bar_width
            for i in range(len(unique_M_MB))
        ]
    )
...
plt.tight_layout()
plt.savefig("lsm_join/plot/6_memory.pdf", bbox_inches="tight", pad_inches=0.02)
plt.close()
