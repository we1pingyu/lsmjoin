...
void generateData(uint64_t s, uint64_t r, double eps_s, double k_r, double k_s,
  ...
  if (!skew) {
    ...
    //   std::cout << "!!!Average number of times each element appears: " <<
    //   average
    //             << " " << frequencyMap.size() << std::endl;
    //   int matches = 0;
    //   for (auto r : R) {
    //     matches += frequencyMap[r];
    //   }
  ...
  void ingest_pk_data(uint64_t tuples, DB * db,
    ...
    for (uint64_t i = 0; i < tuples; i++) {
      ...
      tmp_key =
          string(PRIMARY_SIZE - min(PRIMARY_SIZE, int(tmp.length())), '0') +
          tmp;
  ...
  double build_covering_composite_index(
      DB * db, DB * index, uint64_t * data, const std::vector<uint64_t> &pk,
      uint64_t tuples, int TOTAL_VALUE_SIZE, int SECONDARY_SIZE,
      int PRIMARY_SIZE, double &sync_time, double &update_time,
      bool non_covering) {
  ...
  double build_covering_lazy_index(
      DB * db, DB * index, uint64_t * data, const std::vector<uint64_t> &pk,
      uint64_t tuples, int TOTAL_VALUE_SIZE, int SECONDARY_SIZE,
      int PRIMARY_SIZE, double &sync_time, double &update_time,
      ...
      if (s.ok()) {
        ...
        if (s.ok()) {
          ...
          if (value_split.empty()) {
            Timer timer4 = Timer();
            index->Delete(write_options,
                          tmp_secondary.substr(0, SECONDARY_SIZE));
  ...
  void build_lazy_index(DB * db, DB * index, uint64_t * data,
    ...
    for (uint64_t i = 0; i < tuples; i++) {
      ...
      tmp_primary = use_pk ? std::to_string(pk[i]) : std::to_string(i);
      secondary_key = string(SECONDARY_SIZE - min(SECONDARY_SIZE,
                                                  int(tmp_secondary.length())),
                             '0') +
                      tmp_secondary;
      value =
          string(PRIMARY_SIZE - min(PRIMARY_SIZE, int(tmp_primary.length())),
  ...
  double build_covering_eager_index(
      DB * db, DB * index, uint64_t * data, const std::vector<uint64_t> &pk,
      uint64_t tuples, int TOTAL_VALUE_SIZE, int SECONDARY_SIZE,
      int PRIMARY_SIZE, double &sync_time, double &update_time,
      double &post_list_time, double &eager_time, bool non_covering) {
  ...
  void build_eager_index(
      DB * db, DB * index, uint64_t * data, const std::vector<uint64_t> &pk,
      uint64_t tuples, int TOTAL_VALUE_SIZE, int SECONDARY_SIZE,
      int PRIMARY_SIZE, double &update_time, double &eager_time) {
