...
int SSTDumpTool::Run(int argc, char const* const* argv, Options options) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strncmp(argv[i], "--env_uri=", 10) == 0) {
      ...
      command = argv[i] + 10;
    } else if (sscanf(argv[i], "--key_size=%lu%c", (unsigned long*)&n, &junk) ==
               1) {
      SECONDARY_SIZE = n;
  ...
  for (size_t i = 0; i < filenames.size(); i++) {
    ...
    if (command == "decode") {
      std::string out_filename = filename.substr(0, filename.length() - 4);
      out_filename.append("_decode.txt");
      std::cout << "SECONDARY_SIZE:" << SECONDARY_SIZE << std::endl;
      // std::vector<std::pair<int, int>>* index_block;
      // st = dumper. (out_filename, SECONDARY_SIZE, index_block);
      if (!st.ok()) {
        fprintf(stderr, "%s: %s\n", filename.c_str(), st.ToString().c_str());
        exit(1);
      } else {
        fprintf(stdout, "decode dump written to file %s\n", &out_filename[0]);
      }
      continue;
    }
