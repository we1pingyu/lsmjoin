...
Status BlockBasedTable::DecodeBlock(
    WritableFile* out_file, int SECONDARY_SIZE,
    std::vector<std::pair<int, int>>& index_block,
  ...
  std::unique_ptr<InternalIteratorBase<IndexValue>> blockhandles_iter(
      NewIndexIterator(read_options, /*need_upper_bound_check=*/false,
                       /*input_iter=*/nullptr, /*get_context=*/nullptr,
                       /*lookup_contex=*/nullptr));
  std::unique_ptr<InternalIteratorBase<IndexValue>> data_blockhandles_iter(
      NewIndexIterator(read_options, /*need_upper_bound_check=*/false,
                       /*input_iter=*/nullptr, /*get_context=*/nullptr,
                       /*lookup_contex=*/nullptr));
  size_t block_id = 0;
  for (blockhandles_iter->SeekToFirst(), data_blockhandles_iter->SeekToFirst();
       blockhandles_iter->Valid() && data_blockhandles_iter->Valid();
    ...
    Status tmp_status;
    datablock_iter.reset(NewDataBlockIterator<DataBlockIter>(
        read_options, data_blockhandles_iter->value().handle,
        /*input_iter=*/nullptr, /*type=*/BlockType::kData,
        /*get_context=*/nullptr, /*lookup_context=*/nullptr,
        /*prefetch_buffer=*/nullptr, /*for_compaction=*/false,
        /*async_read=*/false, tmp_status));
    for (datablock_iter->SeekToFirst(); datablock_iter->Valid();
         datablock_iter->Next()) {
      // out_stream << datablock_iter->value().ToString().substr(0,
      // SECONDARY_SIZE)
      //            << "\n";
      block_.emplace_back(
          datablock_iter->value().ToString().substr(0, SECONDARY_SIZE));
    }
    data_block.emplace_back(block_);
  }
