...
for i, label in enumerate(unique_labels):
        ...
        ax.plot(unique_cache_size, sum_join_time, marker=marker, fillstyle='none', linewidth=0.5, markersize=4, color='black')  # 绘制曲线
...
for i, b in enumerate(unique_cache_size):
        for j, label in enumerate(unique_labels):
            ...
            color = 'black'
            # 绘制条形图
            ax2.plot(unique_cache_size, fp * 100, marker=marker, fillstyle='none', linewidth=0.5, markersize=4, color='black', linestyle='--')  # 绘制曲线
...
legend_handles = [
    ...
    mlines.Line2D([], [], color='black', linestyle='--', label='Cache Hit Rate', linewidth=0.5),
    mlines.Line2D([], [], color='black', marker='o', linestyle='None', markersize=4, fillstyle='none', label='2CComp-ISJ'),
    mlines.Line2D([], [], color='black', marker='s', linestyle='None', markersize=4, fillstyle='none', label='P-INLJ'),
]
