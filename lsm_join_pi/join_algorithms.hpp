    ...
    while (it_r->Valid() && it_s->Valid()) {
        ...
        if (temp_r_key == temp_s_key) {
          ...
          if (IsCoveringIndex(config.r_index) || !IsIndex(config.r_index)) {
            ...
            for (auto x : value_split) {
              status = context.db_r->Get(ReadOptions(),
      ...
      void SortMergeForComp(ExpConfig & config, ExpContext & context,
        ...
        void SortMergeForComp(ExpConfig & config, ExpContext & context,
          ...
          while (it_r->Valid() && it_s->Valid()) {
            ...
            if (temp_r_key == temp_s_key) {
              ...
              if (IsCoveringIndex(config.r_index) || !IsIndex(config.r_index)) {
                ...
                // DebugPrint("R: non-covering index");
                status =
                    context.db_r->Get(ReadOptions(), temp_r_value, &value_r);
                if (status.ok() &&
            ...
            if (IsIndex(config.r_index)) {
              ...
              if (IsIndex(config.r_index)) {
                ...
                while (it_r->Valid()) {
                  ...
                  if (!it_r->Valid()) break;
                  temp_r_key = it_r->key().ToString().substr(0, SECONDARY_SIZE);
                  temp_r_value = it_r->key().ToString().substr(SECONDARY_SIZE,
                                                               PRIMARY_SIZE);
                  if (temp_r_key == tmp) {
                    if (IsCoveringIndex(config.r_index)) {
                      ...
                      if (IsCoveringIndex(config.r_index)) {
                        ...
                        status = context.db_r->Get(ReadOptions(), temp_r_value,
                                                   &value_r);
                        if (status.ok() &&
                            value_r.substr(0, SECONDARY_SIZE) == temp_r_key)
                          count1++;
                ...
                if (IsIndex(config.s_index)) {
                  ...
                  while (it_s->Valid()) {
                    ...
                    if (!it_s->Valid()) break;
                    temp_s_key =
                        it_s->key().ToString().substr(0, SECONDARY_SIZE);
                    temp_s_value = it_s->key().ToString().substr(SECONDARY_SIZE,
                                                                 PRIMARY_SIZE);
                    if (temp_s_key == tmp) {
                      if (IsCoveringIndex(config.s_index)) {
                        count2++;
                      } else {
                        status = context.db_s->Get(ReadOptions(), temp_s_value,
                                                   &value_s);
                        if (status.ok() &&
                            value_s.substr(0, SECONDARY_SIZE) == temp_s_key)
                          count1++;
          ...
          void SortMerge(ExpConfig & config, ExpContext & context,
                         RunResult & run_result, rocksdb::Iterator * it_r,
            ...
            if (IsLazyIndex(config.r_index) || IsEagerIndex(config.r_index) ||
              ...
              SortMergeForEagerLazy(config, context, run_result, it_r, it_s,
                                    matches, get_time, val_time);
            } else if (IsCompIndex(config.r_index) ||
                       IsCompIndex(config.s_index)) {
          ...
          else if (temp_r_key < temp_s_key) it_r->Next();
          else if (temp_r_key > temp_s_key) it_s->Next();
        }
      }
      ...
      void NonIndexExternalSortMerge(ExpConfig & config, ExpContext & context,
        ...
        void NonIndexExternalSortMerge(ExpConfig & config, ExpContext & context,
          ...
          int matches = 0;
          ifstream in_r(
              output_file_r);  // File on which sorting needs to be applied
          ifstream in_s(
              output_file_s);  // File on which sorting needs to be applied
          if (!in_r.is_open()) {
          ...
          if (!getline(in_s, line_s) || !getline(in_r, line_r)) {
            DebugPrint("!getline(in_r, line_r): " +
                       to_string(!getline(in_r, line_r)));
            DebugPrint("!getline(in_s, line_s): " +
                       to_string(!getline(in_s, line_s)));
          ...
          while (line_s != "" && line_r != "") {
            ...
            if (getline(iss_r, temp_r_key, ',') &&
                getline(iss_r, temp_r_value) &&
                getline(iss_s, temp_s_key, ',') &&
                getline(iss_s, temp_s_value)) {
              if (temp_r_key == temp_s_key) {
                Timer timer2 = Timer();
        ...
        void SingleIndexExternalSortMerge(
            ExpConfig & config, ExpContext & context, RunResult & run_result,
            rocksdb::Iterator * it_s) {
          cout << "external sort merge" << endl;
          ...
          string output_file_s =
              "/tmp/output_s_" + config.GetTimeStamp() + ".txt";
          string prefix_r = "/tmp/output_r_" + config.GetTimeStamp();
          string output_file_r = prefix_r + ".txt";
          string output_file_s =
              "/tmp/output_s_" + config.GetTimeStamp() + ".txt";
          ...
          while (it_s->Valid() && line_r != "") {
            ...
            if (getline(iss_r, temp_r_key, ',') &&
                ...
                if (getline(iss_r, temp_r_key, ',') &&
                  ...
                  if (temp_r_key == temp_s_key) {
                    while (getline(in_r, line_r)) {  // read next line_r
                      ...
                      value_split = boost::split(value_split, temp_s_value,
                                                 boost::is_any_of(":"));

                      if (IsCoveringIndex(config.s_index) ||
                          !IsIndex(config.s_index)) {
                        // DebugPrint("S: covering or no index");
                        ...
                        if (config.s_index == IndexType::CComp) {
                          ...
                          while (it_s->Valid()) {
                            it_s->Next();
                            if (!it_s->Valid()) break;
                            temp_s_key = it_s->key().ToString().substr(
                                0, SECONDARY_SIZE);
                            temp_s_value = it_s->key().ToString().substr(
                        ...
                        if (IsEagerIndex(config.s_index) ||
                            IsLazyIndex(config.s_index)) {
                          Timer timer2 = Timer();
                          for (auto x : value_split) {
                            status = context.db_s->Get(
                                ReadOptions(), x.substr(0, PRIMARY_SIZE), &tmp);
                            if (status.ok() &&
                          ...
                          while (it_s->Valid()) {
                            ...
                            if (temp_s_key == tmp) {
                              Timer timer2 = Timer();
                              status = context.db_s->Get(
                                  ReadOptions(), temp_s_value, &value_s);
                              if (status.ok() &&
                                  value_s.substr(0, SECONDARY_SIZE) ==
                                      temp_s_key)
                                count2++;
                              val_time += timer2.elapsed();
                            } else
              ...
              void NestedLoop(ExpConfig & config, ExpContext & context,
                              RunResult & result) {
                Timer timer = Timer();
                ReadOptions read_options;
                cout << "joining ... " << endl;
                ...
                rocksdb::Iterator* it_r =
                    context.db_r->NewIterator(read_options);
                rocksdb::Iterator* it_s =
                    context.db_s->NewIterator(read_options);
              ...
              void IndexNestedLoop(ExpConfig & config, ExpContext & context,
                                   RunResult & result, bool covering = true) {
                cout << "index nested loop joining..." << endl;
                string secondary_key_lower, secondary_key_upper, value, tmp_r,
                    tmp_primary;
                int PRIMARY_SIZE = config.PRIMARY_SIZE,
                    SECONDARY_SIZE = config.SECONDARY_SIZE,
                    VALUE_SIZE = config.VALUE_SIZE;
                ...
                rocksdb::Iterator* it_r =
                    context.db_r->NewIterator(read_options);
                rocksdb::Iterator* it_s =
                    context.ptr_index_s->NewIterator(read_options);
                ...
                if (IsEagerIndex(config.s_index) ||
                  ...
                  for (it_r->SeekToFirst(); it_r->Valid(); it_r->Next()) {
                    ...
                    if (s.ok()) {
                      value_split = boost::split(value_split, value,
                                                 boost::is_any_of(":"));
                      ...
                      if (covering) {
                        ...
                        for (auto x : value_set) {
                          s = context.db_s->Get(
                              read_options, x.substr(0, PRIMARY_SIZE), &tmp);

                          if (s.ok() && tmp.substr(0, SECONDARY_SIZE) == tmp_r)
                            matches++;
                        }
                  ...
                  for (it_r->SeekToFirst(); it_r->Valid(); it_r->Next()) {
                    ...
                    for (; it_s->Valid() &&
                           it_s->key().ToString() <= secondary_key_upper;
                         it_s->Next()) {
                      timer1 = Timer();
                      string tmp_s =
                          it_s->key().ToString().substr(0, SECONDARY_SIZE);
                      ...
                      if (tmp_s == tmp_r) {
                        if (!covering) {
                          ...
                          s = context.db_s->Get(
                              ...
                              &value);
                          if (s.ok() &&
                              value.substr(0, SECONDARY_SIZE) == tmp_s)
                            matches++;
                          val_time += timer.elapsed();
                        } else
                          matches++;
